...
### SAMLアサーションのマッピング
* SAML アサーションを受け取るアプリケーションでは、異なるユーザーのメタデータとロールが必要または求める場合がある
  * Keycloakでは、正確に何が転送されるかを定義することができる
  * ロール、クレーム、カスタム属性をハードコードすることができる
  * ユーザー・メタデータをトークンまたはアサーションに取り込むことができます。ロールの名前を変更できる
  * 基本的に、正確に何がクライアントに戻るのかをコントロールできる

### ロール・スコープ・マッピング
* OIDC アクセストークンまたは SAML アサーションが作成されると、ユーザーのすべてのユーザー・ロール・マッピングは、デフォルトでトークンまたはアサーション内にクレームとして追加される
* ロール・スコープ・マッピング は、アクセストークン内で宣言されるロールを制限する方法
* クライアントがユーザーの認証を要求すると、受け取ったアクセストークンには、クライアントのスコープに対して明示的に指定したロールマッピングのみが含まれる
